...
##Test 1: Populated group, empty title, empty article
	def test01_pre_open_blog_DraftPopG_DraftEmptyTA
...
##Test 2: Populated group, populated title, empty article
	def test02_pre_open_blog_DraftPopGT_DraftEmptyA
		login $user_1_email, $master_password
		$browser.goto($patch_blogs_pre_open_article_new)
...
##Test 3: Populated group, empty title, populated article
	def test03_pre_open_blog_DraftPopGA_DraftEmptyT
		login $user_1_email, $master_password
		$browser.goto($patch_blogs_pre_open_article_new)
...
##Test 4: Empty group, populated title, empty article
	def test04_pre_open_blog_DraftPopTA_DraftEmptyG
		login $user_1_email, $master_password
		$browser.goto($patch_blogs_pre_open_article_new)
		sleep 2
...
##Test 5: Empty group, empty title, empty article
	def test05_pre_open_blog_DraftEmptyGN
		login $user_1_email, $master_password
		$browser.goto($patch_blogs_pre_open_article_new)
		sleep 2
