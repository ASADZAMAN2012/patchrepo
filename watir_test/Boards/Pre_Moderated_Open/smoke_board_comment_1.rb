...
##Test 1: One Note Inline Comment
	def test01_pre_open_board_NoteOneComment_TC_24319
		login $user_1_email, $master_password
		$browser.goto($patch_boards_pre_open_note)
		
		commentPopSubmit "Test Comment #{random}"
	end

##Test 2: One Article Inline Comment
	def test02_pre_open_board_ArticleOneComment_TC_24319
		login $user_1_email, $master_password
		$browser.goto($patch_boards_pre_open_article)
		
		commentPopSubmit "Test Comment #{random}"
	end
	
# ##Test 3: One Media Inline Comment
# 	def test03_pre_open_board_MediaOneComment_TC_24319
# 		login $user_1_email, $master_password
# 		$browser.goto($patch_boards_pre_open_article)
# 		wait_for_ajax
# 		sleep 3
...
##Test 4: Two Article Inline Comments
	def test04_pre_open_board_ArticleTwoComments
		login $user_1_email, $master_password
		$browser.goto($patch_boards_pre_open_article)
		commentPopSubmit "Test Comment #{random}"
		commentPopSubmit "Test Comment #{random}"
...
##Test 5: Two Note Inline Comments
	def test05_pre_open_boards_NoteTwoComments_TC_24319
		login $user_1_email, $master_password
		$browser.goto($patch_boards_pre_open_note)
		commentPopSubmit "Test Comment #{random}"
		commentPopSubmit "Test Comment #{random}"
