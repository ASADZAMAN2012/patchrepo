...
##Test 1: Populated group, empty title, empty article
	def test01_post_open_news_DraftPopG_DraftEmptyTA
...
##Test 2: Populated group, populated title, empty article
	def test02_post_open_news_DraftPopGT_DraftEmptyA
		login $user_1_email, $master_password
		$browser.goto($patch_news_post_open_article_new)
...
##Test 3: Populated group, empty title, populated article
	def test03_post_open_news_DraftPopGA_DraftEmptyT
		login $user_1_email, $master_password
		$browser.goto($patch_news_post_open_article_new)
...
##Test 4: Empty group, populated title, empty article
	def test04_post_open_news_DraftPopTA_DraftEmptyG
		login $user_1_email, $master_password
		$browser.goto($patch_news_post_open_article_new)
		sleep 2
...
##Test 5: Empty group, empty title, empty article
	def test05_post_open_news_DraftEmptyGN
		login $user_1_email, $master_password
		$browser.goto($patch_news_post_open_article_new)
		sleep 2
